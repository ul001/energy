bui.ready(function () {
    ...
    function getMainData(curid) {
        ...
        energyObj.getDataByAjax("GET", "/api/app/AppPlatform/GetByBuildID", param, function (data) {
            if (data.error) {
                return;
            }
            var html = "";
            ...
            var timeStr = "-";
            if (data.lastSum) {
                yesVal = data.lastSum.value;
            }
            if (data.thisSum) {
                todayVal = data.thisSum.value;
            }
            try {
            ...
            //能源类型
            if (data.energyCount) {
                $("#energyType").text(data.energyCount);
            }
            //网关数量
            if (data.collectionCount.allDevices) {
                $("#gatewayNum").text(data.collectionCount.allDevices);
            }
            //仪表数量
            if (data.meterCount.allDevices) {
                $("#meterNum").text(data.meterCount.allDevices);
            }
            var codeStr = $(".secord-class .bui-btn.selected").attr("data-value");
            var consumeType = "今日用能";
            switch (codeStr) {
                case "01000":
                    consumeType = "今日用电";
                    unitStr = "kW·h";
